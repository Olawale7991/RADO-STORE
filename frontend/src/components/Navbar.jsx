...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-0 font-medium'>
        ...
        <NavLink to='/contact' className='flex  flex-col items-center gap-1 font-bold'>
          <p>CONTACT</p>
          <hr className='w-2/4 border-none h-[1.5px] bg-gray-700 hidden'/>
        </NavLink>
        </ul>
        ...
        <div className='flex items-center gap-6'>
          ...
          <Link to='/card' className='relative'>
                ...
                <img onClick={() =>setvisible(true)} src={assets.dropdown} className='w-5 cursor-pointer sm:hidden' alt="" />
        </div>
        {/* sidebar menu for small screen */}
        <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
                <div className='flex flex-col text-gray-600'>
                  <div onClick={() => setvisible(false)} className='flex items-center gap-4 p-3 cursor-pointer'>
                    <img src={assets.cross} className='h-4 rotate-180' alt="" />
                    <p>Back</p>
                  </div>
